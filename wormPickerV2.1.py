...
class statusClass(ttk.Frame):#var=self.status
	def __init__(self, addBox, controller): 
		...
		self.statusDict = {'None':self.No, 'Maintaining':self.Ma, 'Imaged':self.Im, 'Standby':self.St, 
		...
		self.StCheck = tk.Checkbutton(self.checkFrame, text = 'Standby', variable = self.St ).grid(row = 3, sticky='w')
...
class strainsToPick(ttk.Frame):
	...
	def calcDaysSince(self, todayDate, datePicked):
		...
		try:
			daysSince = todayDate - datetime.datetime.strptime(datePicked, '%Y-%m-%d')
			...
			try:
				...
				try:
					daysSince = todayDate - datetime.datetime.strptime(datePicked, '%m/%d/%Y')
	...
	def calcDaysUntil(self, todayDate, remindDate):
		...
		try:
			daysUntil = datetime.datetime.strptime(remindDate, '%Y-%m-%d') - todayDate
			...
			try:
				...
				try:
					daysUntil = datetime.datetime.strptime(remindDate, '%m/%d/%Y') - todayDate
...
class ControllerWin(ttk.Frame): #In most cases, parent != controller. Controller references the controller win, 
	...
	def enterData(self):
		...
		if self.strainEntry == None or self.strainEntry == '':
			...
			#self.viewPage.dataWin.savetocsv()
...
def initDB():
	...
	if path.exists('WormStrainsData.csv') == False:
		...
		blankrow = ['Delete this row', '2022-01-01', '2022-01-01', 'None', 'None', 'None', '3', 'None', '2022-01-01']
