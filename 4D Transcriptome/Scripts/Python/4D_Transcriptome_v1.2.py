'''
3rd iteration of 4DT (2D + T) viewing.
Allow for single gene but have capability in mind for multiple genes or iterative workflow
...
def main():
	...
	mainDraw(cells, seamCells, maxTPM, image_output_dir, imgDim)
...
def norm_to_RGB(cells): 
	'''
	normalizes the TPM for all cells to 0 -> 1. 
	Used for fill colors, which will sort normalized TPM values into color bins

	'''

	maxTPM = 0
	for cell in cells: 
		#print(cell.TPM_loess['tpm'])
		cellMax = float(cell.TPM_loess['tpm'].max())
		if cellMax > float(maxTPM):
			maxTPM = float(cellMax)
		else: 
			continue

	for cell in cells: 
		cell.TPM_loess['intensity'] = cell.TPM_loess['tpm'] / maxTPM
		#print(cell.TPM_loess.head())
	return maxTPM


def fillColors(norm_val, assoc=False):
...
def mainDraw(cells, seamCells, maxTPM, image_output_dir, imgDim, radius = 5, bgcolor = (100,100,255)): 

	for cell in cells: 
		print(cell.coord_data.head())
		# cell.coord_data['RGB'] = cell.coord_data.apply(lambda x: fillColors(x['intensity'], x['association']), axis = 1)
		cell.coord_data['RGB'] = cell.TPM_loess.apply(lambda x: fillColors(x['intensity']), axis = 1)
	...
	for t in range(0, len(cells[0].coord_data["x"])):
		...
		drawScaleBar(draw, imgDim, radius, maxTPM)
...
def drawScaleBar(draw, imgDim, maxTPM, radius):
	'''
	...
	y2 = int(0.60*imgDim[1])

	#shape keys

	#Max and min labels
	draw.text((x1, y1-10), "{} TPM".format(maxTPM)) #-30 for two lines, -5 for one line
	draw.text((x1, y2+5), "0 TPM")
	caption = 'Scale bar represents the normalized percent of maximum expression.'
