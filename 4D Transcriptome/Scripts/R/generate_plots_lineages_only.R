...
bdc_copy <- bdc[!is.na(bdc$lineage),]
...
for (gene in genes) {
  ...
  split_df <- split(mean_tpm_table, mean_tpm_table$lineage)
  ...
  setwd(sessionDir)
  filename <- glue("{outputgene}_lin_plots.pdf")
  ggsave(
    filename = filename, 
...
cells <- c("ABaraaaap", "ABpxppapa", "MSxppa", "Z2/Z3:pseudotime_bin_2")

ntable <- mean_tpm_table[mean_tpm_table$lineage %in% cells,]
ntable <- ntable[!is.na(ntable$mean_tpm),]
split_df <- split(ntable, ntable$lineage)

setwd(outputDir)
outputgene <- as.character(gene)
outputfolder <- "plots_se"
dir.create(file.path(getwd(), outputfolder))
setwd(glue("{getwd()}\\{outputfolder}"))
sessionDir <- getwd()
plots <- list()
...
setwd(sessionDir)
filename <- glue("{outputgene}_lin_plots_se_bigger.pdf")
ggsave(
  filename = filename, 
  plot = marrangeGrob(plots, nrow=1, ncol=2), 
  width = 15, height = 9
)
