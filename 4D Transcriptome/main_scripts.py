...
def main():
	...
	genes = ["efn-2", "efn-3", "efn-4", "wrk-1", "vab-1", "nck-1", "dab-1", "ran-1", "daf-18","vab-2" ]
	...
	for gene in genes:
		...
		image_output_dir = smooth_gene_output+'/{}_visualization'.format(gene)
...
def getGene(GSE_gene):
	...
	while True:
		...
		if genename in GSE_gene['gene_short_name'].unique():
			break
		elif os.path.exists("output/{}/{}".format(datetime.date.today(), genename)) == False:
			print("\nData for {} is not found in 'output/{}/{}'. Please run analyses in R or try again.".format(genename, datetime.date.today(), genename))
		else:
