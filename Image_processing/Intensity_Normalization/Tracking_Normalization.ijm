...
Dialog.addString("Deep learning model for untwisting channel", DL_model);
...
og_image_a = Path + "\\"+ DL_model +"_" + uc + "\\" +uc_prefix+ "_reg_"+start+".tif";
open(og_image_a);
IDA = getImageID();
...
og_image_b = Path + "\\RCAN_2Step_DL_" + tc + "\\" +tc_prefix+ "_reg_"+start+".tif";
open(og_image_b);
IDB = getImageID();
run("Z Project...", "projection=[Max Intensity]");
IDBP = getImageID();
...
image_uc = Path + "\\"+ DL_model+ "_" + uc + "\\"+uc_prefix+"_reg_"+i+".tif";
open(image_uc);
ID1 = getImageID();
...
run("Scale...", "x=1.0 y=1.0 z=.987 width=425 height=325 depth=308 interpolation=Bilinear average process create title=Decon_reg_"+i+".tif");
