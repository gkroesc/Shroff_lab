...
import argparse
#import tflearn
import rln.makedata3D_train_single as Input
import rln.makedata3D_test_single as InputT
from rln.makedata3D_test_single import normalize
import random
import tqdm
import math
import itertools
...
train_model_path='C:/Users/kroeschellg/Downloads/Worm_untwisting_project/Image_processing/Deep_Learning/RedUntwistingModel/'
train_output='C:/Users/kroeschellg/Desktop/DL_TrainingV2/train/output_rl/'

data_dir = 'X:/shrofflab/Efn-4_RedUntwisting/Tracking/031723/EFN-4_RedUT_A/Pos4/SPIMB_91-119/Reg_Sample/'
input_folder='RegB/'

test_output= data_dir + 'RLNDecon_DL_' + input_folder + '/'

train_iter_num=100*100#100*100 #epoch*iteration
test_iter_num=120 #test number
train_batch_size=4
...
input_labels=os.listdir(data_dir + input_folder)
predition_file = data_dir + input_folder + '/'+ input_labels[0]
...
#            time_start=time.time()
            ...
            for m in range(test_iter_num):
                ...
                max_v,test_images,label2out,shape=InputT.get_data_tiff(data_dir,input_folder,test_batch_size,normal_pmin,normal_pmax)
                ...
                for v in range(test_batch_size):
                    single = normal_reshape_image
                    #single=reshape_image[v]*65535
                    filenames_out=test_output+'DL_'+label2out[v]
                    tiff.imsave(filenames_out,single.astype('uint16'), imagej=True)
                #########  save output image #####################
                if m % 1 == 0:
                    time_end=time.time()
